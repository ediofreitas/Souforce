...
export default class ShauryaComponent extends LightningElement {
    ...
    connectedCallback() {
        if(this.isSymptom){
            ...
            /*this.title = 'Symptom Classification';
            this.label = 'Help us train Dell Insights by classifying the case'
            this.fieldLevelHelp = 'Dell Insights is our Artificial Intelligence(AI) here to assist you with tasks. It helps by classifying the case so we can identify ways to better serve our customers.'*/
        }else{
            this.getResolutionData();
            /*this.title = 'Resolution Classification';
            this.label = 'What are the resolution classifications?'*/
        }
        //this.getAssetType();
        this.getUserDetails();
    }
    ...
    handleChange(event) {
        ...
        if (this.searchTerm.length > 2) {
            if(this.isSymptom){
                this.makeServercallSymptom();
            }else{
                this.makeServercallResolution();
            }
        } else {
            this.showValues = false;
            this.valuesopen = false;
